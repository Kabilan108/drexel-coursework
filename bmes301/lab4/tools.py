...
def load_data(filepath):
    """
    Load data from MTS
    Zeros out the time and deflection columns
    """

    # Check file header
    with open(filepath, 'r') as f:
        lines = f.readlines()
    assert lines[6].rstrip() == 'Crosshead \tLoad \tTime', "Unexpected file header"
    assert lines[7].rstrip() == 'mm\tN\tsec', "Unexpected file header"

    # Load data
    df = (pd.read_table(filepath, sep='\t', skiprows=8, header=None)
        .rename(columns={0: 'deflect', 1: 'force', 2: 'time'})
        .astype(float))

    # Zero out the time and deflection
    df['deflect'] = df['deflect'] - df['deflect'].iloc[0]
